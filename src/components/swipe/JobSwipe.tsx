...
export default function JobSwipe({ jobs, isAuthed }: { jobs: SwipeJob[]; isAuthed: boolean }) {
  ...
  const advance = useCallback(
    (dir: Dir) => {
      ...
      setLeaving(dir)
      // wait for the fly-off animation, then move to the next card
      window.setTimeout(() => {
        setLeaving(null)
        setDrag({ x: 0, y: 0 })
        setIndex((i) => i + 1)
  ...
  const flyX = leaving === 'right' ? 600 : leaving === 'left' ? -600 : drag.x
  const flyY = drag.y
  const rotate = flyX / 18
  ...
  return (
    <div className="flex w-full max-w-sm flex-col items-center gap-7">
      <div className="relative h-[500px] w-full select-none">
        ...
        {next && (
          <div className="absolute inset-0 scale-[0.94] opacity-60">
        ...
        {current && (
          <div
            ...
            style={{
              transform: `translate(${flyX}px, ${flyY}px) rotate(${rotate}deg)`,
